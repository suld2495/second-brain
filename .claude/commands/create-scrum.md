...
- Project Key: `SC`
- Issue Type: `작업` (id: 10003)

### 2. Story 정보 조회 (MCP)
`mcp__Atlassian__getJiraIssue` 호출하여 상위 Story 존재 여부 확인:
```json
{
...
3. 설명 추출: 아래 항목을 포함하도록 변환합니다:
   - 범위 (scope: in/out)
   - 수용 기준 (acceptance criteria)
   - 테스트 계획

### 4. Jira Task 생성 (apply 모드만, MCP)
...
**상위 Story**: <story-key> - <story-summary>
**프로젝트**: SC (지식베이스)
**이슈 타입**: 작업
**제목**: <추출한 제목>
...
### 확인
- `--plan` 없이 다시 실행하면 Task가 생성됩니다.
```

## 참고
- 로컬 파일은 생성하지 않습니다. Jira가 단일 소스입니다.
- Scrum은 실행 가능한 가장 작은 작업 단위입니다.
- `/start-scrum`으로 작업을 시작하면 Jira 상태가 "In Progress"로 변경됩니다.
